...
class Bandit:
    ...
    def run(self):
        ...
        for period in range(1, self.n_periods+1):
            ...
            P_ab_ctr, _ = self.bayes.p_ab_loss( [beta(log[variant]['alpha'], log[variant]['beta']) for variant in self.agent.variants] )
            P_ab_cpc, _ = self.bayes.p_ab_loss( [gamma(log[variant]['a'], log[variant]['scale']) for variant in self.agent.variants] )
            ...
            for a, b in itertools.combinations(self.agent.variants, 2):    
                ...
                _, loss['ctr'][a, b] = self.bayes.p_ab_loss( [beta(log[a]['alpha'],log[a]['beta']), beta(log[b]['alpha'],log[b]['beta'])] )
                _, loss['cpc'][a, b] = self.bayes.p_ab_loss( [gamma(log[a]['a'],log[a]['scale']), gamma(log[b]['a'],log[b]['scale'])] )
